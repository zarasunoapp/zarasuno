...
import {
  Coins, Check, Star, CreditCard, QrCode, Ticket, Sparkles, ShieldCheck, Upload, Info,
} from "lucide-react";
...
function Inner({ packages, configs, country }: { packages: CoinPackage[]; configs: PaymentConfig[]; country: string }) {
  ...
  const redeem = () => {
    ...
    addCoins(50);
    flash(`Promo "${promo.toUpperCase()}" redeemed · +50 coins`);
    setPromo("");
  };
    ...
    <div className="mx-auto max-w-5xl px-4 py-10 pb-28 sm:px-6 md:pb-10">
      ...
      <div className="mt-4 space-y-3">
        ...
        >
          ...
          <span className="hidden rounded-full bg-brand px-4 py-2 text-sm font-semibold text-white sm:inline">
            Pay {pkg.currency} {pkg.price.toLocaleString()}
          </span>
        </button>
      ...
      <div className="mt-8 rounded-3xl paper p-6 shadow-soft ring-1 ring-brand-100/50">
        ...
        <div className="mt-3 flex gap-2">
          ...
          />
          <button onClick={redeem} className="rounded-full bg-brand px-6 py-3 text-sm font-semibold text-white hover:bg-brand-600">Redeem</button>
        </div>
      </div>
      ...
      {method && <ManualPaymentModal config={method} pkg={pkg} onClose={() => setMethod(null)} onDone={(m) => { setMethod(null); flash(m); }} />}

      {showCard && (
        <StripeCardModal
          pkg={pkg}
          onClose={() => setShowCard(false)}
          onSuccess={async (m) => { setShowCard(false); await refresh(); flash(m); }}
        />
      )}
...
function ManualPaymentModal({ config, pkg, onClose, onDone }: { config: PaymentConfig; pkg: CoinPackage; onClose: () => void; onDone: (msg: string) => void }) {
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/50 p-0 backdrop-blur-sm sm:items-center sm:p-4" onClick={onClose}>
      <div className="max-h-[90vh] w-full max-w-md animate-fade-up overflow-y-auto rounded-t-3xl bg-white p-6 shadow-2xl sm:rounded-3xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="mt-4 rounded-2xl border border-gray-100 p-4 text-center">
          ...
          <p className="mt-1 text-lg font-bold text-brand-700">Send {pkg.currency} {pkg.price.toLocaleString()}</p>
        </div>
        ...
        <button disabled={!ref.trim()} onClick={() => onDone("Payment submitted — we'll add your coins after verifying. Thank you!")} className="btn-green mt-5 w-full rounded-full py-3.5 font-semibold disabled:opacity-40">
          Submit payment proof
        </button>
        <p className="mt-2 text-center text-xs text-gray-400">Creates a pending transaction · admin approves &amp; credits coins</p>
      </div>
    </div>
  );
}
